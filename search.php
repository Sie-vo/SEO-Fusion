...
function search_striphtmlbbcodes(string $text) {
	$text = preg_replace("[\[(.*?)\]]", "", $text);
	$text = preg_replace("<\<(.*?)\>>", "", $text);
	return $text;
}

function search_textfrag(string $text) {
	if ($_GET['chars'] != 0) {
...
function search_stringscount(string $text) {
	global $swords;
...
function search_querylike(string $field) {
	global $swords;
...
function search_globalarray(string $search_result) {
...
function search_navigation(int $rows) {
