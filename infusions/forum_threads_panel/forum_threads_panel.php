...
if (dbrows($result)) {
	while($data = dbarray($result)) {
		...
		echo THEME_BULLET." <a href='".seo_url('forum_thread', $data['thread_id'])."' title='".$data['thread_subject']."' class='side'>$itemsubject</a><br />\n";
	}
} else {
...
if (dbrows($result) != 0) {
	...
	while($data = dbarray($result)) {
		...
		echo "<tr>\n<td class='side-small'>".THEME_BULLET." <a href='".seo_url('forum_thread', $data['thread_id'])."' title='".$data['thread_subject']."' class='side'>$itemsubject</a></td>\n";
