 ...
 * @var string $message
 * @var mixed $disable_smileys_check
 * @var mixed $sig_checked
 * @var mixed $notify_checked
 */
...
if (isset($_POST['postreply'])) {
		...
		if ($message != "") {
			...
			if (!flood_control("post_datestamp", DB_POSTS, "post_author='".$userdata['user_id']."'")) {
				...
				if ($fdata['forum_attach'] && checkgroup($fdata['forum_attach'])) {
					...
					redirect(seo_url('forum_forum', $_GET['forum_id']));
			}
		} else {
			$error = 3;
		}
	...
	if (isset($_GET['quote']) && isnum($_GET['quote'])) {
		...
		if (dbrows($result)) {
			$data = dbarray($result);
			$message = "[quote][url=".seo_url('forum_thread', $_GET['thread_id'], ['pid' => $_GET['quote']])."#post_".$_GET['quote']."][b]".$data['user_name'].$locale['429']."[/b][/url]\n\n".strip_bbcodes($data['post_message'])."[/quote]";
		}
	}
