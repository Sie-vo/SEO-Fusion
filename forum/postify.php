...
if (($_GET['post'] == "on" || $_GET['post'] == "off") && $settings['thread_notify']) {
	...
	if (dbrows($result)) {
		...
		if (checkgroup($data['forum_access'])) {
			add_to_head("<meta http-equiv='refresh' content='2; url=".seo_url('forum_thread', $_GET['thread_id'])."' />\n");
			$output = true;
			...
			echo "<a href='".seo_url('forum_thread', $_GET['thread_id'])."'>".$locale['447']."</a> ::\n";
			echo "<a href='".seo_url('forum_forum', $_GET['forum_id'])."'>".$locale['448']."</a> ::\n";
	...
	if ($_GET['error'] < "2") {
		...
		add_to_head("<meta http-equiv='refresh' content='2; url=".seo_url('forum_thread', $_GET['thread_id'], ['pid' => $_GET['post_id']])."#post_".$_GET['post_id']."' />\n");
		if ($settings['thread_notify']) {
			...
			if (dbrows($result)) {
				...
				$link = seo_url('forum_thread', $_GET['thread_id'], ['pid' => $_GET['post_id']])."#post_".$_GET['post_id'];
		...
		echo "<a href='".seo_url('forum_thread', $_GET['thread_id'], ['pid' => $_GET['post_id']])."#post_".$_GET['post_id']."'>".$locale['447']."</a> ::\n";
	} else {
		if (!isset($_GET['thread_id']) || !isnum($_GET['thread_id'])) { redirect("index.php"); }
		$data = dbarray(dbquery("SELECT post_id FROM ".DB_POSTS." WHERE thread_id='".$_GET['thread_id']."' ORDER BY post_id DESC"));
		add_to_head("<meta http-equiv='refresh' content='4; url=".seo_url('forum_thread', $_GET['thread_id'], ['pid' => $data['post_id']])."#post_".$data['post_id']."' />\n");
		echo "<a href='".seo_url('forum_thread', $_GET['thread_id'], ['pid' => $data['post_id']])."#post_".$data['post_id']."'>".$locale['447']."</a> ::\n";
	}
	echo "<a href='".seo_url('forum_forum', $_GET['forum_id'])."'>".$locale['448']."</a> ::\n";
	echo "<a href='index.php'>".$locale['449']."</a><br /><br />\n</div>\n";
	closetable();
