...
if ($rows > $threads_per_page || (iMEMBER && $can_post)) {
	$forum_url = seo_url('forum_forum', $_GET['forum_id']);
	$post_info .= "<table cellspacing='0' cellpadding='0' width='100%'>\n<tr>\n";
	if ($rows > $threads_per_page) {
		$post_info .= "<td style='padding:4px 0px 4px 0px'>";
		$post_info .= makepagenav($_GET['rowstart'],$threads_per_page,$rows,3,$forum_url.(strpos($forum_url, '?') === false ? '?' : '&amp;'));
		$post_info .= "</td>\n";
	}
	if (iMEMBER && $can_post) {
...
while ($data2 = dbarray($result)) {
	...
	$forum_list .= "<option value='".seo_url('forum_forum', $data2['forum_id'])."'".$sel.">".$data2['forum_name']."</option>\n";
}
$forum_list .= "</optgroup>\n";
...
echo "document.location.href=forum_url;\n";
echo "</script>\n";
