...
if (isset($_POST['delete_posts']) && isset($_POST['delete_post']) && is_array($_POST['delete_post']) && count($_POST['delete_post'])) {
	...
	if (!$thread_count) { redirect(seo_url('forum_forum', $fdata['forum_id'])); }
		...
		if (dbrows($move_result)) {
			...
			if ($f_post) {
				...
				if ($dell_f_post && count($array_post) == 1) {
					...
					echo "<a href='".seo_url('forum_thread', $pdata['thread_id'], ['rowstart' => $_GET['rowstart']])."'>".$locale['609']."</a>";
					$f_post_blo = true;
				}	
				echo "</div></div>\n";
			}
			if (!isset($_POST['new_forum_id']) && !$f_post_blo) {
				...
				if (dbrows($fl_result) > 0) {
					...
					echo "<a href='".seo_url('forum_thread', $pdata['thread_id'], ['rowstart' => $_GET['rowstart']])."'>".$locale['609']."</a><br /><br />\n";
					echo "</div></div>";
				}
					...
					if (dbrows($tl_result) > 0) {
						...
						echo "<a href='".seo_url('forum_thread', $pdata['thread_id'])."'>".$locale['609']."</a>\n";
						echo "</div></div><br />\n";
					}
				...
				if (!dbcount("(thread_id)", DB_THREADS, "thread_id='".$_POST['new_thread_id']."' AND forum_id='".$_POST['new_forum_id']."'")) {
					redirect(seo_url('forum_thread', $fdata['thread_id'], ['rowstart' => $_GET['rowstart'], 'error' => 1]));
				}
				...
				if ($move_posts_add) {
					...
					if ($posts_ex) {	
						...
						redirect(seo_url('forum_thread', $_POST['new_thread_id'], ['pid' => $array_post[$pid]])."#post_".$array_post[$pid]);
					} else { 
						redirect(seo_url('forum_thread', $fdata['thread_id'], ['rowstart' => $_GET['rowstart'], 'error' => 2]));
					}
				} else { 
					redirect(seo_url('forum_thread', $fdata['thread_id'], ['rowstart' => $_GET['rowstart'], 'error' => 3]));
				}
			} else { 
				redirect(seo_url('forum_index', 0));	
				closetable();
			}
		} else { 
			redirect(seo_url('forum_thread', $fdata['thread_id'], ['rowstart' => $_GET['rowstart'], 'error' => 2]));
	...
	if ($message != "") {
		...
		echo "<a href='".seo_url('forum_thread', $fdata['thread_id'], ['rowstart' => $_GET['rowstart']])."'>".$locale['609']."</a><br />";
		echo "</div></div>\n";
		closetable();
		require_once THEMES."templates/footer.php";
		die();
