 ...
 * @var array $locale
 * @var array $settings
 * @var mixed $fusion_images
 * @var array $userdata
 * @var mixed $lastvisited
...
$thread_url = seo_url('forum_thread', $_GET['thread_id']);
...
$forum_url = seo_url('forum_forum', $fdata['forum_id']);
...
if (($rows > $posts_per_page) || ($can_post || $can_reply)) {
	...
	if ($rows > $posts_per_page) { echo "<td style='padding:4px 0px 4px 0px'>".makepagenav($_GET['rowstart'],$posts_per_page,$rows,3,$thread_url.(strpos($thread_url, '?') === false ? '?' : '&amp;'))."</td>\n"; }
	if (iMEMBER && $can_post) {
...
while ($data = dbarray($result)) {
	...
	$forum_list .= "<option value='".seo_url('forum_forum', $data['forum_id'])."'$sel>".$data['forum_name']."</option>\n";
}
$forum_list .= "</optgroup>\n";
if (iMOD) {
...
$viewthread_js .= "function jumpforum(forum_url){";
$viewthread_js .=    "document.location.href=forum_url;";
$viewthread_js .= "}";
