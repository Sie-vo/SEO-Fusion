...
for ($i = 0; $i < $qcount; $i++) {
	...
	$text = preg_replace_callback('#'.$exta.'(.*?)\[/quote\]#si', function ($matches) use ($before, $endbefore, $after, $locale) {
		$post_id = (int)$matches[2];
		$thread_id = dbresult(dbquery("SELECT thread_id FROM ".DB_POSTS." WHERE post_id='".$post_id."'"), 0);
		$href = $thread_id ? seo_url('forum_thread', $thread_id, ['pid' => $post_id])."#post_".$post_id : '#';
		return $before."<a class='quote-link' rel='".$post_id."' href='".$href."' target='_blank'>".$matches[1]." ".$locale['bb_wrote'].": <span class='goto-post-arrow'>&uarr;</span></a>".$endbefore.$matches[3].$after;
	}, $text); //replace quote with valid name and post //HTML 
