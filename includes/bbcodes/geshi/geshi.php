 ...
 * @coAuthor      Dennis Vorpahl info@sievo.de
...
if (!function_exists('stripos')) {
		...
		function stripos(string $haystack,string $needle, $offset = NULL) {
...
class GeSHi {
	 ...
	 * @var array
	 */
	public $error_messages = array(GESHI_ERROR_NO_SUCH_LANG => 'GeSHi could not find the language {LANGUAGE} (using path {PATH})',
	 ...
	 * @var array
	 */
	public $lexic_permissions = array('KEYWORDS' => array(), 'COMMENTS' => array('MULTI' => TRUE), 'REGEXPS' => array(),
								   'ESCAPE_CHAR' => TRUE, 'BRACKETS' => TRUE, 'SYMBOLS' => FALSE, 'STRINGS' => TRUE,
								   'NUMBERS' => TRUE, 'METHODS' => TRUE, 'SCRIPT' => TRUE);
	/**
	...
	public function error() {
		if ($this->error) {
			...
			$debug_tpl_vars = array('{LANGUAGE}' => $this->language, '{PATH}' => $this->language_path);
	 ...
	 * @return array
	 */
	public function get_supported_languages(bool $full_names = FALSE) {
		// return array
		$back = [];
	...
	public function get_language_name_from_extension($extension, $lookup = []) {
		if (!is_array($lookup) || empty($lookup)) {
			$lookup = array('6502acme' => array('a', 's', 'asm', 'inc'), '6502tasm' => array('a', 's', 'asm', 'inc'),
							'6502kickass' => array('a', 's', 'asm', 'inc'),
							'68000devpac' => array('a', 's', 'asm', 'inc'), 'abap' => array('abap'),
							'actionscript' => array('as'), 'ada' => array('a', 'ada', 'adb', 'ads'),
							'apache' => array('conf'), 'asm' => array('ash', 'asm', 'inc'), 'asp' => array('asp'),
							'bash' => array('sh'), 'bf' => array('bf'), 'c' => array('c', 'h'),
							'c_mac' => array('c', 'h'), 'caddcl' => array(), 'cadlisp' => array(),
							'cdfg' => array('cdfg'), 'cobol' => array('cbl'),
							'cpp' => array('cpp', 'hpp', 'C', 'H', 'CPP', 'HPP'), 'csharp' => array('cs'),
							'css' => array('css'), 'd' => array('d'), 'delphi' => array('dpk', 'dpr', 'pp', 'pas'),
							'diff' => array('diff', 'patch'), 'dos' => array('bat', 'cmd'),
							'gdb' => array('kcrash', 'crash', 'bt'), 'gettext' => array('po', 'pot'),
							'gml' => array('gml'), 'gnuplot' => array('plt'), 'groovy' => array('groovy'),
							'haskell' => array('hs'), 'html4strict' => array('html', 'htm'),
							'ini' => array('ini', 'desktop'), 'java' => array('java'), 'javascript' => array('js'),
							'klonec' => array('kl1'), 'klonecpp' => array('klx'), 'latex' => array('tex'),
							'lisp' => array('lisp'), 'lua' => array('lua'), 'matlab' => array('m'), 'mpasm' => array(),
							'mysql' => array('sql'), 'nsis' => array(), 'objc' => array(), 'oobas' => array(),
							'oracle8' => array(), 'oracle10' => array(), 'pascal' => array('pas'),
							'perl' => array('pl', 'pm'), 'php' => array('php', 'php5', 'phtml', 'phps'),
							'povray' => array('pov'), 'providex' => array('pvc', 'pvx'), 'prolog' => array('pl'),
							'python' => array('py'), 'qbasic' => array('bi'), 'reg' => array('reg'),
							'ruby' => array('rb'), 'sas' => array('sas'), 'scala' => array('scala'),
							'scheme' => array('scm'), 'scilab' => array('sci'), 'smalltalk' => array('st'),
							'smarty' => array(), 'tcl' => array('tcl'), 'vb' => array('bas'), 'vbnet' => array(),
							'visualfoxpro' => array(), 'whitespace' => array('ws'), 'xml' => array('xml', 'svg', 'xrc'),
	 ...
	 * @since 1.0.0
	 */
	public function remove_keyword_group(int $key) {
	 ...
	 * @since 1.0.8
	 */
	public function optimize_keyword_group(int $key) {
		...
		if ($space_as_whitespace) {
			foreach ($this->language_data['CACHED_KEYWORD_LISTS'][$key] as $rxk => $rxv) {
				$this->language_data['CACHED_KEYWORD_LISTS'][$key][$rxk] = str_replace(" ", "\\s+", $rxv);
			}
		}
	}
	 ...
	 */
	public function highlight_lines_extra($lines, $style = '') {
		if (is_array($lines)) {
			...
			if ($style === '') { //Check if we should use default style
				unset($this->highlight_extra_lines_styles[$lines]);
			} else if ($style === '') { //Check if to remove this line
		...
		if ($this->lexic_permissions['NUMBERS']) {
			...
			if (is_array($this->language_data['NUMBERS'])) {
				...
				if (!$this->language_data['NUMBERS']) {
					$this->language_data['NUMBERS'] = GESHI_NUMBER_INT_BASIC | GESHI_NUMBER_FLT_NONSCI;
				}
				for ($i = 0, $j = $this->language_data['NUMBERS']; $j > 0; ++$i, $j >>= 1) {
					...
					if (isset($this->language_data['STYLES']['NUMBERS'][1 << $i])) {
						$this->language_data['STYLES']['NUMBERS'][$i] = $this->language_data['STYLES']['NUMBERS'][1 << $i];
	...
	private function build_parse_cache() {
		...
		foreach (array_keys($this->language_data['KEYWORDS']) as $key) {
			if (!isset($this->lexic_permissions['KEYWORDS'][$key]) || $this->lexic_permissions['KEYWORDS'][$key]) {
				$this->optimize_keyword_group($key);
			}
		}
		// brackets
		if ($this->lexic_permissions['BRACKETS']) {
			...
			if (!$this->use_classes && isset($this->language_data['STYLES']['BRACKETS'][0])) {
				$this->language_data['CACHE_BRACKET_REPLACE'] = array('<| style="'.$this->language_data['STYLES']['BRACKETS'][0].'">&#91;|>',
				...
				$this->language_data['CACHE_BRACKET_REPLACE'] = array('<| class="br0">&#91;|>',
																	  '<| class="br0">&#93;|>',
																	  '<| class="br0">&#40;|>',
																	  '<| class="br0">&#41;|>',
																	  '<| class="br0">&#123;|>',
		...
		if ($this->lexic_permissions['NUMBERS']) {
			...
			static $numbers_format = array(GESHI_NUMBER_INT_BASIC => '(?:(?<![0-9a-z_\.%$@])|(?<=\.\.))(?<![\d\.]e[+\-])([1-9]\d*?|0)(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_INT_CSTYLE => '(?<![0-9a-z_\.%])(?<![\d\.]e[+\-])([1-9]\d*?|0)l(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_BIN_SUFFIX => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])[01]+?[bB](?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_BIN_PREFIX_PERCENT => '(?<![0-9a-z_\.%])(?<![\d\.]e[+\-])%[01]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_BIN_PREFIX_0B => '(?<![0-9a-z_\.%])(?<![\d\.]e[+\-])0b[01]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_OCT_PREFIX => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])0[0-7]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_OCT_PREFIX_0O => '(?<![0-9a-z_\.%])(?<![\d\.]e[+\-])0o[0-7]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_OCT_PREFIX_AT => '(?<![0-9a-z_\.%])(?<![\d\.]e[+\-])\@[0-7]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_OCT_SUFFIX => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])[0-7]+?o(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_HEX_PREFIX => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])0x[0-9a-fA-F]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_HEX_PREFIX_DOLLAR => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])\$[0-9a-fA-F]+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_HEX_SUFFIX => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])\d[0-9a-fA-F]*?[hH](?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_FLT_NONSCI => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])\d+?\.\d+?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_FLT_NONSCI_F => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])(?:\d+?(?:\.\d*?)?|\.\d+?)f(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_FLT_SCI_SHORT => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])\.\d+?(?:e[+\-]?\d+?)?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)',
										   GESHI_NUMBER_FLT_SCI_ZERO => '(?<![0-9a-z_\.])(?<![\d\.]e[+\-])(?:\d+?(?:\.\d*?)?|\.\d+?)(?:e[+\-]?\d+?)?(?![0-9a-z]|\.(?:[eE][+\-]?)?\d)');
			...
			foreach ($this->language_data['NUMBERS_CACHE'] as $key => $rxdata) {
				...
				$this->language_data['NUMBERS_RXCACHE'][$key] = "/(?<!<\|\/)(?<!<\|!REG3XP)(?<!<\|\/NUM!)(?<!\d\/>)($regexp)(?!(?:<DOT>|(?>[^\<]))+>)(?![^<]*>)(?!\|>)(?!\/>)/i"; //
			}
	 ...
	 * @since 1.0.0
	 * @todo Internal limitation: utilizes too many types inferring and code completition might not provide complete results
	 */
	public function parse_code() {
		...
		if ($this->strict_mode) {
			...
			while ($i < $length) {
					...
					if (is_array($delimiters)) {
						foreach ($delimiters as $open => $close) {
							...
							if (!isset($matches[$dk][$open])) {
								$matches[$dk][$open] = array('next_match' => -1, 'dk' => $dk, 'open' => $open,
															 // needed for grouping of adjacent code blocks (see below)
															 'open_strlen' => strlen($open), 'close' => $close,
						...
						if (!GESHI_PHP_PRE_433 && //Needs proper rewrite to work with PHP >=4.3.0; 4.3.3 is guaranteed to work.
							...
							if (isset($matches_rx['start']) && isset($matches_rx['end'])) {
								$matches[$dk] = array('next_match' => $matches_rx['start'][1], 'dk' => $dk,
													  'close_strlen' => strlen($matches_rx['end'][0]),
													  'close_pos' => $matches_rx['end'][1],);
							} else {
								$matches[$dk] = array('next_match' => $matches_rx[1][1], 'dk' => $dk,
				...
				$parts[$k] = [1 => substr($code, $i, $next_match_pos-$i)];
				++$k;
				...
				for ($i = 0; $i < $length; ++$i) {
					...
					if (isset($this->language_data['COMMENT_REGEXP']) && $next_comment_regexp_pos < $i) {
						...
						foreach ($this->language_data['COMMENT_REGEXP'] as $comment_key => $regexp) {
							...
							} else if (//This is to allow use of the offset parameter in preg_match and stay as compatible with older PHP versions as possible
								(GESHI_PHP_PRE_433 && preg_match($regexp, substr($part, $i), $match, PREG_OFFSET_CAPTURE)) || (!GESHI_PHP_PRE_433 && preg_match($regexp, $part, $match, PREG_OFFSET_CAPTURE, $i))
							) {
								$match_i = $match[0][1];
								if (GESHI_PHP_PRE_433) {
									$match_i += $i;
								}
								$comment_regexp_cache_per_key[$comment_key] = array('key' => $comment_key,
					...
					if ($string_started && ($i != $next_comment_regexp_pos)) {
						...
						do {
								...
								if (isset($this->language_data['ESCAPE_REGEXP']) && $next_escape_regexp_pos < $start) {
									...
									foreach ($this->language_data['ESCAPE_REGEXP'] as $escape_key => $regexp) {
										...
										if (isset($escape_regexp_cache_per_key[$escape_key]) && ($escape_regexp_cache_per_key[$escape_key]['pos'] >= $start || $escape_regexp_cache_per_key[$escape_key]['pos'] === FALSE)) {
											...
											(GESHI_PHP_PRE_433 && preg_match($regexp, substr($part, $start), $match, PREG_OFFSET_CAPTURE)) || (!GESHI_PHP_PRE_433 && preg_match($regexp, $part, $match, PREG_OFFSET_CAPTURE, $start))
										) {
											$match_i = $match[0][1];
											if (GESHI_PHP_PRE_433) {
												$match_i += $start;
											}
											$escape_regexp_cache_per_key[$escape_key] = array('key' => $escape_key,
																							  'length' => strlen($match[0][0]),
																							  'pos' => $match_i);
										} else {
							...
							if ($simple_escape < $next_escape_regexp_pos && $simple_escape < $length && $simple_escape < $close_pos) {
								...
								if ($es_char == "\n") {
									...
									if (function_exists('mb_substr')) {
										...
										if (preg_match("/[\xC2-\xDF][\x80-\xBF]"."|\xE0[\xA0-\xBF][\x80-\xBF]"."|[\xE1-\xEC\xEE\xEF][\x80-\xBF]{2}"."|\xED[\x80-\x9F][\x80-\xBF]"."|\xF0[\x90-\xBF][\x80-\xBF]{2}"."|[\xF1-\xF3][\x80-\xBF]{3}"."|\xF4[\x80-\x8F][\x80-\xBF]{2}/s", $part, $es_char_m, 0, $es_pos+1)) {
							...
							} else if ($next_escape_regexp_pos < $length && $next_escape_regexp_pos < $close_pos) {
						...
						$i = $start-1;
						continue;
					} else if ($this->lexic_permissions['STRINGS'] && $hq && $hq[0] == $char && substr($part, $i, $hq_strlen) == $hq && ($i != $next_comment_regexp_pos)) {
						...
						while ($close_pos = strpos($part, $this->language_data['HARDQUOTE'][1], $start)) {
							$start = $close_pos+1;
							if ($this->lexic_permissions['ESCAPE_CHAR'] && $part[$close_pos-1] == $this->language_data['HARDCHAR'] && (($i+$hq_strlen) != ($close_pos))) { //Support empty string for HQ escapes if Starter = Escape
						...
						if (!$COMMENT_MATCHED) {
							...
							if (!empty($this->language_data['COMMENT_SINGLE']) && $next_comment_single_pos < $i) {
								...
								foreach ($this->language_data['COMMENT_SINGLE'] as $comment_key => $comment_mark) {
									...
									} else if (// case sensitive comments
										($this->language_data['CASE_SENSITIVE'][GESHI_COMMENTS] && ($match_i = stripos($part, $comment_mark, $i)) !== FALSE) || // non case sensitive
										(!$this->language_data['CASE_SENSITIVE'][GESHI_COMMENTS] && (($match_i = strpos($part, $comment_mark, $i)) !== FALSE))
									) {
	 ...
	 * @param  array $match The matches array
	...
	private function handle_keyword_replace($match) {
		...
		if ($this->keyword_links) {
			...
			if (isset($this->language_data['URLS'][$k]) && $this->language_data['URLS'][$k] != '') {
				...
				if (!$this->language_data['CASE_SENSITIVE'][$k] && strpos($this->language_data['URLS'][$k], '{FNAME}') !== FALSE) {
				...
				$before = '<|UR1|"'.str_replace(array('{FNAME}', '{FNAMEL}', '{FNAMEU}',
													  '.'), array(str_replace('+', '%20', urlencode($this->hsc($word))),
																  str_replace('+', '%20', urlencode($this->hsc(strtolower($word)))),
																  str_replace('+', '%20', urlencode($this->hsc(strtoupper($word)))),
																  '<DOT>'), $this->language_data['URLS'][$k]).'">';
				$after = '</a>';
			}
		}
	...
	private function handle_multiline_regexps($matches) {
		...
		return $before.'<|!REG3XP'.$this->_hmr_key.'!>'.str_replace("\n", "|>\n<|!REG3XP".$this->_hmr_key.'!>', $replace).'|>'.$after;
	}
	...
	private function parse_non_string_part($stuff_to_parse) {
		...
		foreach (array_keys($this->language_data['KEYWORDS']) as $k) {
			if (!isset($this->lexic_permissions['KEYWORDS'][$k]) || $this->lexic_permissions['KEYWORDS'][$k]) {
				...
				if ($parser_control_pergroup && isset($this->language_data['PARSER_CONTROL']['KEYWORDS'][$k])) {
					if (isset($this->language_data['PARSER_CONTROL']['KEYWORDS'][$k]['DISALLOWED_BEFORE'])) {
						$disallowed_before_local = $this->language_data['PARSER_CONTROL']['KEYWORDS'][$k]['DISALLOWED_BEFORE'];
					}
					if (isset($this->language_data['PARSER_CONTROL']['KEYWORDS'][$k]['DISALLOWED_AFTER'])) {
						$disallowed_after_local = $this->language_data['PARSER_CONTROL']['KEYWORDS'][$k]['DISALLOWED_AFTER'];
					}
				}
				...
				for ($set = 0, $set_length = count($this->language_data['CACHED_KEYWORD_LISTS'][$k]); $set < $set_length; ++$set) {
					...
					$stuff_to_parse = preg_replace_callback("/$disallowed_before_local({$keywordset})(?!\<DOT\>(?:htm|php|aspx?))$disallowed_after_local/$modifiers", array($this,
																																											'handle_keyword_replace'), $stuff_to_parse);
				}
			}
		}
		// Regular expressions
			...
			if ($this->lexic_permissions['REGEXPS'][$key]) {
				if (is_array($regexp)) {
					if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
						...
						$stuff_to_parse = preg_replace_callback("/".$regexp[GESHI_SEARCH]."/{$regexp[GESHI_MODIFIERS]}", array($this,
					...
					if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
						// produce valid HTML when we match multiple lines
						$this->_hmr_key = $key;
						$stuff_to_parse = preg_replace_callback("/(".$regexp.")/", array($this,
																						 'handle_multiline_regexps'), $stuff_to_parse);
						$this->_hmr_key = '';
					} else {
		...
		foreach (array_keys($this->language_data['KEYWORDS']) as $k) {
			if (!$this->use_classes) {
				$attributes = ' style="'.(isset($this->language_data['STYLES']['KEYWORDS'][$k]) ? $this->language_data['STYLES']['KEYWORDS'][$k] : "").'"';
			} else {
		...
		/**  TODO: Fix lexic permissions not converting entities if shouldn't be highlighting regardless */
		//
		if ($this->lexic_permissions['BRACKETS']) {
			$stuff_to_parse = str_replace($this->language_data['CACHE_BRACKET_MATCH'], $this->language_data['CACHE_BRACKET_REPLACE'], $stuff_to_parse);
		}
		...
		if ($this->lexic_permissions['SYMBOLS'] && !empty($this->language_data['SYMBOLS'])) {
			...
			for ($s_id = 0; $s_id < $n_symbols; ++$s_id) {
				...
				if (strpos($symbol_match, '<') !== FALSE || strpos($symbol_match, '>') !== FALSE) {
					...
					if (strpos($symbol_match, '<SEMI>') === FALSE && strpos($symbol_match, '<PIPE>') === FALSE) {
						continue;
					}
				}
				...
				if ($this->language_data['MULTIPLE_SYMBOL_GROUPS']) {
					...
					foreach ($sym_match_syms[0] as $sym_ms) {
						...
						if (isset($this->language_data['SYMBOL_DATA'][$sym_ms]) && ($this->language_data['SYMBOL_DATA'][$sym_ms] != $old_sym)) {
							if (-1 != $old_sym) {
								$symbol_hl .= "|>";
							}
		...
		foreach (array_keys($this->language_data['REGEXPS']) as $key) {
			if ($this->lexic_permissions['REGEXPS'][$key]) {
				if (is_callable($this->language_data['STYLES']['REGEXPS'][$key])) {
					$this->_rx_key = $key;
					$stuff_to_parse = preg_replace_callback("/!REG3XP$key!(.*)\|>/U", array($this,
																							'handle_regexps_callback'), $stuff_to_parse);
				} else {
					if (!$this->use_classes) {
	...
	private function finalise(&$parsed_code) {
		...
		if ($this->enable_important_blocks && (strpos($parsed_code, $this->hsc(GESHI_START_IMPORTANT)) === FALSE)) {
					...
					for ($i = 0; $i < $n; ++$i) {
						...
						if ($this->line_numbers == GESHI_FANCY_LINE_NUMBERS && $i%$this->line_nth_row == ($this->line_nth_row-1)) {
							...
							if ($this->use_classes) {
								...
								// so that styles applied to special lines don't apply to the actual
								// code on that line
								$parsed_code .= '<span style="display:block;'.$this->line_style2.'">'.'<span style="'.$this->code_style.'">';
							}
							$close += 2;
						}
						//Is this some line with extra styles???
	...
	private function header() {
		...
		if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
			...
			} else if ($this->header_type == GESHI_HEADER_DIV || $this->header_type == GESHI_HEADER_PRE_VALID) {
			...
			if ($this->header_type == GESHI_HEADER_PRE) {
				return "<pre$attributes>$header".($this->force_code_block ? '<div>' : '');
			} else {
				return "<div$attributes>$header".($this->force_code_block ? '<div>' : '');
			}
		}
	}
	...
	function footer() {
		...
		if ($this->header_type == GESHI_HEADER_DIV || $this->header_type == GESHI_HEADER_PRE_VALID) {
			...
			return ($this->force_code_block ? '</div>' : '')."$footer</div>";
		} elseif ($this->header_type == GESHI_HEADER_PRE_TABLE) {
			if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
			...
			return ($this->force_code_block ? '</div>' : '')."$footer</pre>";
		}
	}
	...
	public function _genCSSName(mixed $name) {
	...
	public function get_stylesheet($economy_mode = TRUE) {
		...
		foreach ($this->language_data['STYLES']['KEYWORDS'] as $group => $styles) {
			if ($styles != '' && (!$economy_mode || (isset($this->lexic_permissions['KEYWORDS'][$group]) && $this->lexic_permissions['KEYWORDS'][$group]))) {
				$stylesheet .= "$selector.kw$group {{$styles}}\n";
			}
		}
		foreach ($this->language_data['STYLES']['COMMENTS'] as $group => $styles) {
			if ($styles != '' && (!$economy_mode || (isset($this->lexic_permissions['COMMENTS'][$group]) && $this->lexic_permissions['COMMENTS'][$group]) || (!empty($this->language_data['COMMENT_REGEXP']) && !empty($this->language_data['COMMENT_REGEXP'][$group])))) {
		...
		foreach ($this->language_data['STYLES']['REGEXPS'] as $group => $styles) {
			if ($styles != '' && (!$economy_mode || (isset($this->lexic_permissions['REGEXPS'][$group]) && $this->lexic_permissions['REGEXPS'][$group]))) {
	 ...
	 * @return array for regular expression
		...
		for ($i = 0, $i_max = count($list); $i < $i_max; ++$i) {
			...
			while (TRUE) {
				...
				if (isset($prev_keys[$level])) {
					...
					$char = 0;
					while (isset($entry[$char]) && isset($prev_keys[$level][$char]) && $entry[$char] == $prev_keys[$level][$char]) {
						++$char;
					}
					if ($char > 0) {
						...
						if ($char == strlen($prev_keys[$level])) {
							...
							if (in_array($new_key_part1[0], $regex_chars) || in_array($new_key_part2[0], $regex_chars)) {
								...
								// recreate key index
								array_splice($prev_keys, $level, count($prev_keys), array($new_key_part1,
																						  $new_key_part2));
	 ...
	 * @param bool $recursed bool to know wether we recursed or not
	 * @return string
	 * @author Milian Wolff <mail@milianw.de>
	 * @since  1.0.8
	 * @access private
	 */
	private function _optimize_regexp_list_tokens_to_string(&$tokens, $recursed = FALSE) {
		$list = '';
...
if (!function_exists('geshi_highlight')) {
	 ...
	 * @since 1.0.2
	 */
	 function geshi_highlight($string, $language, $return = FALSE) {
